...
class CoolConsoleWindow(QWidget):
    def __init__(self, root: Path) -> None:
        ...
        self._intro_lines = [
            ...
            "Built-ins: cd, pwd, clear, cls, settings, coolconsole, progressdemo, shell, session",
        ]
    ...
    def _place_settings_panel(self) -> None:
        ...
        self.settings_panel.setGeometry(int(shell.right() - 338), int(shell.top() + self._title_bar_height + 14), 320, 420)
    ...
    def _animate_next_output(self) -> None:
        ...
        line = self.buffer.append_line(text=text, style=style, revealed=0, kind=kind)
        if not text:
            line.revealed = 0
            self._on_line_changed()
    ...
    def _tick_effects(self) -> None:
        ...
        if self.theme.matrix or (self._idle_ticks > 180 and not self.command_runner.is_running()):
            self._advance_matrix()
            changed = True

        if changed or self.theme.crt:
            self.update()
    ...
    def _ensure_matrix_columns(self) -> None:
        ...
        if len(self._matrix_columns) == count:
            return
        self._matrix_columns = [random.uniform(-term_rect.height(), term_rect.height()) for _ in range(count)]
        self._matrix_speeds = [random.uniform(3.0, 10.0) for _ in range(count)]
    ...
    def _content_height(self) -> float:
        line_count = max(1, self.buffer.total_lines() + 1)
        return line_count * (self.text_renderer.line_height + 2) + self._padding * 2
    ...
    def _terminal_rect(self) -> QRectF:
        return QRectF(self._padding, self._title_bar_height + 18, self.width() - self._padding * 2, self.height() - self._title_bar_height - self._padding - 24)
    ...
    def _status_text(self) -> str:
        ...
        return f"{self.theme.shell_mode}  |  {session_text}  |  {cwd_text}"
    ...
    def _build_render_rows(self) -> list[dict]:
        ...
        rows.append(
            {
                "text": f"{self._prompt_prefix()} {self._current_input}",
                "style": "prompt_active",
                "y": y + 10,
    ...
    def _paint_terminal(self, painter: QPainter) -> None:
        ...
        prompt_row = rows[-1]
        prompt_y = prompt_row["y"]
        prompt_text = prompt_row["text"]
        if self.cursor.visible and not self._input_locked():
            prompt_prefix = f"{self._prompt_prefix()} "
            cursor_text = prompt_prefix + self._current_input[: self._input_cursor]
            cursor_x = term_rect.left() + self.text_renderer.text_width(cursor_text) + 4
            cursor_rect = QRectF(cursor_x, prompt_y + 4, 10, self.text_renderer.line_height - 8)
